...
aIk = (RBIk' * ( fBk - bak - vak )) - [0 0 P.constants.g]';
omegaBk = omegaBtildek - bgk - vgk;

rIkp1 = rIk + (vIk * delt) + (delt^2 * aIk/2);
vIkp1 = vIk + (delt * aIk);
ekp1 = ek + (delt * omegaBk);

alpha_a = exp(-delt/P.sensorParams.taua);
alpha_g = exp(-delt/P.sensorParams.taug);

bakp1 = (alpha_a * bak) + va2k;
bgkp1 = (alpha_g * bgk) + vg2k;
