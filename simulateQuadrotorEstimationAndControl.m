function [Q,Ck] = simulateQuadrotorEstimationAndControl(R,S,P)
...
%
% CK ---------- Structure with the following elements:
%
%          Pser = 2x2xN covariance matrix estimate series. Pser(:,:,k) is
%                 the estimated covariance matrix at times tk = tVec(k),
%  
%        Poserr = Mx3 matrix composed such that Poserr(k,:)' is the 3x1
%                 position belief error at tVec(k) in the I frame, in meters.
%+------------------------------------------------------------------------------+
% References:
%
%
% Author:  Todd Humphreys (Radionavigation Laboratory)
%+==============================================================================+  
...
for kk=1:N-1
  ...
  Ck.meas_made(kk) = make_meas;
  
  E = stateEstimatorUKF(Se,M,P,make_meas); 
...
%   disp( R.rIstar(kk,1:2))
  ...
  Ck.Poserr(1:3,kk) = statek.rI - E.statek.rI;
  Ck.Posblf(1:3,kk) = E.statek.rI;
...
%** Do a final measurement update at the end of the trajectory
% Simulate measurements
statek.rI = Xk(1:3);
statek.RBI(:) = Xk(7:15);
statek.vI = Xk(4:6);
...
[M.ftildeB,M.omegaBtilde] = imuSimulator(Sm,P);
E = stateEstimatorUKF(Se,M,P,true); 
Ck.Pser(1:3,1:3,N) = E.Pk(1:3,1:3);
Ck.Poserr(1:3,N) = statek.rI - E.statek.rI;
Ck.Posblf(1:3,N) = E.statek.rI;
Ck.meas_made(N) = int8(make_meas);
