...
for i = 1:Nf
    if mcVeck(i) ~= 0
        rXIMat_sensed(j,:) = rXIMat(i,:);
        j = j + 1;
    end
end

[Nfk,~] = size(rXIMat_sensed);
vIMatC_h = zeros(3,Nfk);
for i = 1:Nfk
    vIi = (rXIMat_sensed(i,:)' - rcI);
    vIMatC_h(:,i) = P.sensorParams.RCB * RBI * vIi/norm(vIi);
end

rbB = P.sensorParams.rA(:,2) - P.sensorParams.rA(:,1);

rpI_h = xk(1:3) + ( RBI' * P.sensorParams.rA(:,1) );
rbIu_h = RBI' * rbB/norm(rbB);

h = [ rpI_h ; rbIu_h ; reshape(vIMatC_h,[],1) ];
